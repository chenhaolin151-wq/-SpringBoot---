...
public interface AttendanceMapper extends BaseMapper<AttendanceRecord> {
    ...
    @Select("SELECT s.user_id as userId, s.work_date as workDate " +
            "FROM schedule s " +
            "LEFT JOIN attendance_record ar ON s.user_id = ar.user_id " +
            "AND DATE(s.work_date) = DATE(ar.punch_date) " +
            "WHERE DATE_FORMAT(s.work_date, '%Y-%m') = #{month} " +
            "AND s.work_date < CURDATE() " +
            "AND ar.id IS NULL")
    List<Map<String, Object>> findMonthlyMissingRecords(@Param("month") String month);
}